  ...
  Object.keys(factoryState.zones).forEach(zoneId => {
    ...
    if (zone.workers > 0) {
      ...
      if (zone.hasHelmetViolation) {
        ...
        // Clear danger alerts for this zone if violation resolved
        factoryState.alerts = factoryState.alerts.filter(a => !(a.zone === zoneId && a.level === "danger"));
      }
    } else {
      zone.hasHelmetViolation = false;
      // Clear danger alerts for this zone
      factoryState.alerts = factoryState.alerts.filter(a => !(a.zone === zoneId && a.level === "danger"));
    }
    ...
    if (zone.workers >= 4) {
      ...
      const exists = factoryState.alerts.some(a => a.message === msg);
      if (!exists) {
        // Remove old warning alerts for this zone first before adding updated one
        factoryState.alerts = factoryState.alerts.filter(a => !(a.zone === zoneId && a.level === "warning"));
      ...
      // Clear warning alerts for this zone when workers count is back to normal
      factoryState.alerts = factoryState.alerts.filter(a => !(a.zone === zoneId && a.level === "warning"));
    }
  ...
  try {
    ...
    if (aiDecision && aiDecision.recommendations) {
      ...
      if (aiDecision.safetyAlert && aiDecision.safetyAlert.hasAlert) {
        const isHelmet = aiDecision.safetyAlert.message.includes("안전모") || aiDecision.safetyAlert.message.includes("보호구");
        const isCrowded = aiDecision.safetyAlert.message.includes("밀집") || aiDecision.safetyAlert.message.includes("과밀");
        
        const hasSimilarAlert = factoryState.alerts.some(a => {
          if (isHelmet && (a.message.includes("안전모") || a.message.includes("보호구"))) return true;
          if (isCrowded && (a.message.includes("밀집") || a.message.includes("과밀"))) return true;
          return a.message === aiDecision.safetyAlert.message;
        });
